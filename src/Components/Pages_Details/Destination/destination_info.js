...
import DestinationData from "../Data/destination_data.json";
    ...
    const activetext = "h-full border-b-[3px] pb-3 cursor-pointer flex gap-3.5";
    const inactivetext = "h-full hover:border-b-[3px] pb-3 cursor-pointer hover:border-white/[0.5] flex gap-3.5";

function DestinationInfo() {
    const [Data, setData] = useState([1]);
    let currentData = (id) => {
        setData((prev) =>
            prev.includes(id && id===id)
            ...
            <div className="absolute flex text-white top-60 right-72 ">
                {
                    DestinationData.map((info) => (
                        <div key={info.id}>
                            ...
                            {/* Planet list */}
                            <div className={Data.includes(info.id) ? activetext : inactivetext} onClick={() => currentData(info.id)}>
