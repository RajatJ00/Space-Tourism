...
function CrewInfo() {
    const activetext = "w-4 h-4 mr-7 rounded-full bg-white bg-white ";
    const inactivetext = "w-4 h-4 mr-7 rounded-full hover:bg-white bg-white/[.10] ";
    ...
    return (
        <>
            ...
            <div className="absolute flex top-[620px] left-44 mx-4">
                {
                    CrewData.map((info) => (
                        <div className={Data.includes(info.id) ? activetext : inactivetext} onClick={() => currentData(info.id)}></div>))}
            </div>
        </>
    );
